/* is_register_64 binds to a function
@value: a string/the item we are searching for */
var is_register_64 = function(value) {
  /* The indexOf() method searches the register_name_64 array for the
   specified item, and returns its position */
   // If the position/index is >= 0, then value is present in the array
  if (register_name_64.indexOf(value) >= 0) {
    return true;
  }
  // If the position/index is <= 0 (function returned -1), then the value is not present in the array
  return false;

  //Either store true or false after the function executes inside is_register_64 variable
};

/* is_register_32 binds to a function
@value: a string/the item we are searching for */
var is_register_32 = function(value) {
  /* The indexOf() method searches the register_name_32 array for the
   specified item, and returns its position */
   // If the position/index is >= 0, then value is present in the array
  ...
  //Either store true or false after the function executes inside is_register_32 variable
};

// This function receives a 32 bit register and finds the 64 bit equivalent
var register32To64 = function(reg32) {
  // Retrieve the index of the 32 bit register from register_name_32 array
  index = register_name_32.indexOf(reg32);
  // Ensuring that the index is greater than 0 (indicates register exists in the array)
  if (index >= 0) {
    // Use that index to find the 64 bit equivalent in the register_name_64 array
    // Both the 32 bit and 64 bit array place their register in the same position
    // i.e. rbp and ebp are at position 0 in their respective array, so we can use the same index to alternate between different register sizes
